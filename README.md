...
*   **Native Padding:** Полная поддержка протокола набивки пакетов NaiveProxy для защиты от анализа трафика по размеру и таймингам. Сервер и форк [Haonixao/naiveproxy](https://github.com/Haonixao/naiveproxy) поддерживают два режима:
...
*   **`2` (Variant2):** `NumFirstPaddings = -1` — паддинг на **каждой** read/write без ограничения. Доступен только в связке с форком [Haonixao/naiveproxy](https://github.com/Haonixao/naiveproxy) (клиент объявляет поддержку `kVariant2` и согласует тип через заголовки).
